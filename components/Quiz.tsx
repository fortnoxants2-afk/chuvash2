...
const Quiz: React.FC<QuizProps> = ({ 
  ...
  const answersMarginTop = (isLongQuestion || isPeopleIllustration) ? 'mt-4' : (isSpecial ? 'mt-0' : (isRayony ? 'mt-3' : 'mt-6'));
    ...
    <div className="flex flex-col h-full overflow-hidden relative">
      ...
      <div className="absolute z-10 flex flex-col items-center" style={{ width: '93.333%', height: '91.5%', left: '3.333%', bottom: '1.2%' }}>
        ...
        <div className={`relative z-10 w-full h-full flex flex-col items-center px-[10%] pb-[30%] ${contentPaddingTop}`}>
          <div className={`${containerWidthClass} ${isSquareImg ? 'aspect-square' : 'aspect-[3.4/4.3]'} ${imageMarginBottom} relative flex-shrink-0 flex items-center justify-center transition-all duration-300 ${shouldDisableMask ? '' : 'rounded-[2.5rem] overflow-hidden bg-[#f3f4f6] shadow-inner'}`}>
             ...
             {imgStatus === 'error' && (
               <div className="absolute inset-0 flex flex-col items-center justify-center p-4 text-center">
                 ...
                 <span className="text-[#5d3a24] font-bold text-[10px] uppercase opacity-40">Ошибка загрузки</span>
               </div>
             )}
          ...
          <div className="flex-1 w-full px-2 flex flex-col justify-center items-center text-center min-h-[3rem] relative z-20">
            <h2 className={`text-[#0a0a0a] font-bold ${isLongQuestion ? 'text-[16px]' : 'text-[17px]'} leading-tight tracking-tight`}>
              {question.text}
            </h2>
          </div>
          
          <div className={`w-full space-y-0.5 flex-shrink-0 relative z-10 ${answersMarginTop}`}>
            {Object.entries(question.options).map(([key, text]) => (
              <button
                key={key}
                disabled={showFeedback}
                onClick={() => setSelectedOption(key)}
                className={`w-full py-1.5 px-6 text-left rounded-[1.2rem] font-bold text-[#0a0a0a] text-[15px] transition-all duration-150 ${selectedOption === key ? 'bg-white shadow-md scale-[1.02]' : 'bg-transparent active:bg-white/10'}`}
              >
                {text}
              </button>
            ))}
